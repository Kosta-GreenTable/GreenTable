...
public class CartServiceImpl implements CartService {
	...
	public List<CartDTO> selectCartByUserId(int userId) throws SQLException, SelectException {
		...
		if (list == null)
			throw new SelectException("장바구니 정보를 가져오는데 실패했습니다.");
		return list;
	}
	...
	public int insertCart(CartDTO cartdto) throws SQLException, AddException {
		...
		if (result == 0)
			throw new AddException("장바구니 상품 추가에 실패했습니다.");
		return result;
	}
	...
	public Map<String, Object> updateCart(int userId, int productId, int quantity)
		...
		if (updateResult == 0)
			throw new ModifyException("상품 수량 변경에 실패했습니다.");
	...
	/**
	 * 장바구니 가격 정보 계산 총 상품금액, 총 할인금액, 배송비, 결제금액 등을 계산하여 반환
	 */
	private Map<String, Integer> calculatePrices(List<CartDTO> cartItems) {
	...
	public Map<String, Object> calculateCartPrices(int userId) throws SQLException, SelectException {
		...
		Map<String, Integer> priceMap = calculatePrices(cartList);

		Map<String, Object> resultMap = new HashMap<>(priceMap);
	...
	public Map<String, Object> calculateSelectedProducts(int userId, List<Integer> productIds)
		...
		List<CartDTO> cartItems = selectCartByUserId(userId).stream()
				.filter(item -> productIds.contains(item.getProductId())).collect(Collectors.toList());

		Map<String, Integer> priceMap = calculatePrices(cartItems);
		Map<String, Object> resultMap = new HashMap<>(priceMap);
		return resultMap;
	}

	@Override
	public Map<String, Object> deleteCart(int userId, int productId) throws SQLException, DeleteException {
		Map<String, Object> resultMap = new HashMap<>();

		int deleteResult = cartDao.deleteCart(userId, productId);

		if (deleteResult == 0)
			throw new DeleteException("상품 삭제에 실패했습니다.");
	...
	@Override
	public List<CartDTO> getGuestCartItems(List<CartDTO> guestCartItems) throws SQLException {

		for (CartDTO item : guestCartItems) {
			Product product = productDao.selectProductDetail(item.getProductId());
			if (product != null) {
				// 상품 정보로 CartDTO 채우기
				item.setProductName(product.getName());
				item.setPrice(product.getPrice());
				item.setDiscountRate(product.getDiscountRate());
				item.setImageName(product.getMainImageName()); // 메인 이미지
			}
		}
	...
	public Map<String, Object> calculateGuestCartPrices(List<CartDTO> guestCartItems) throws SQLException {
		Map<String, Integer> priceMap = calculatePrices(guestCartItems);
