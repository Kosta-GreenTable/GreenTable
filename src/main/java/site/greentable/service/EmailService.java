...
public class EmailService {
    ...
    private void loadConfig() {
        ...
        try {
            // 환경변수 우선, 없으면 properties 파일에서 읽기
            String mailUsername = System.getenv("MAIL_USERNAME");
            String mailPassword = System.getenv("MAIL_PASSWORD");
            String mailHost = System.getenv("MAIL_SMTP_HOST");
            String mailPort = System.getenv("MAIL_SMTP_PORT");
            ...
            if (mailHost != null && !mailHost.isEmpty()) {
                config.setProperty("mail.smtp.host", mailHost);
            }
            if (mailPort != null && !mailPort.isEmpty()) {
                config.setProperty("mail.smtp.port", mailPort);
            }

            // 환경변수가 없으면 properties 파일에서 읽기
            if (mailUsername == null || mailUsername.isEmpty() ||
                ...
                if (input != null) {
                    ...
                    // 환경변수가 없는 것만 파일에서 읽기
                    if (mailUsername == null || mailUsername.isEmpty()) {
                        config.setProperty("mail.username", fileProps.getProperty("mail.username", ""));
                    }
                    if (mailPassword == null || mailPassword.isEmpty()) {
                        config.setProperty("mail.password", fileProps.getProperty("mail.password", ""));
                    }
                    if (mailHost == null || mailHost.isEmpty()) {
                        config.setProperty("mail.smtp.host", fileProps.getProperty("mail.smtp.host", "smtp.gmail.com"));
                    }
                    if (mailPort == null || mailPort.isEmpty()) {
                        config.setProperty("mail.smtp.port", fileProps.getProperty("mail.smtp.port", "587"));
                    }

                    // 기본 SMTP 설정들
                    config.setProperty("mail.smtp.auth", fileProps.getProperty("mail.smtp.auth", "true"));
                    config.setProperty("mail.smtp.starttls.enable",
                            fileProps.getProperty("mail.smtp.starttls.enable", "true"));
                    config.setProperty("mail.smtp.ssl.trust",
                            fileProps.getProperty("mail.smtp.ssl.trust", "smtp.gmail.com"));
                ...
                config.setProperty("mail.smtp.ssl.trust", "smtp.gmail.com");
            }

            System.out.println("이메일 설정 로드 완료 - Host: " + config.getProperty("mail.smtp.host"));
