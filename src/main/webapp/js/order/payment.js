document.addEventListener("DOMContentLoaded", () =>{
    ...
    function requestPayment(merchantUid) {
        ...
        // 포트원 결제 요청
        const { IMP } = window;
        IMP.init('imp54237185'); // 가맹점 식별코드는 서버에서 안전하게 관리, 여기서는 샘플코드
    ...
    async function verifyPayment(rsp) {
        try {
            ...
            const response = await fetch(`${contextPath}/ajax?key=orderRest&methodName=verifyPayment`, {
                ...
                body: `impUid=${rsp.imp_uid}&merchantUid=${rsp.merchant_uid}&paidAmount=${rsp.paid_amount}`
            });
            ...
            if (data.success) {
                ...
                document.getElementById("payBtn").textContent = "결제 완료";
