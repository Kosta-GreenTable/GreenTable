...
try {
    // 데이터베이스 연결
    String url = "jdbc:mysql://localhost:3306/greentable?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC";
    ...
    // 6. 최근 등록된 상품 5개 조회
    try {
        pstmt = conn.prepareStatement("SELECT p.product_id, p.name, pi.image_name as main_image_name, p.price, pd.created_date as created_at FROM products p LEFT JOIN product_images pi ON p.product_id = pi.product_id AND pi.is_main = 1 LEFT JOIN product_details pd ON p.product_id = pd.product_id ORDER BY pd.created_date DESC LIMIT 5");
        rs = pstmt.executeQuery();
        while (rs.next()) {
            Map<String, Object> product = new HashMap<>();
            product.put("productId", rs.getInt("product_id"));
            product.put("name", rs.getString("name"));
            product.put("mainImageName", rs.getString("main_image_name"));
            product.put("price", rs.getInt("price"));
            product.put("createdAt", rs.getDate("created_at"));
    ...
    // 7. 최근 등록된 농가 5개 조회 (created_at 컬럼이 없으므로 farm_id로 정렬)
    try {
        pstmt = conn.prepareStatement("SELECT farm_id, name, description, farm_img FROM farms ORDER BY farm_id DESC LIMIT 5");
        rs = pstmt.executeQuery();
        while (rs.next()) {
            ...
            farm.put("farmImg", rs.getString("farm_img"));
            farm.put("createdAt", null); // created_at 컬럼이 없으므로 null
          ...
          <div class="dashboard-stats">
            <div class="stat-card">
              ...
              <div class="stat-content">
                <h3>총 회원 수</h3>
                <p class="stat-number"><fmt:formatNumber value="${totalUsers}" pattern="#,###" /></p>
              </div>
            </div>
            ...
            <div class="stat-card">
              ...
              <div class="stat-content">
                <h3>등록된 상품</h3>
                <p class="stat-number"><fmt:formatNumber value="${totalProducts}" pattern="#,###" /></p>
              </div>
            </div>
            ...
            <div class="stat-card">
              ...
              <div class="stat-content">
                <h3>등록된 농가</h3>
                <p class="stat-number"><fmt:formatNumber value="${totalFarms}" pattern="#,###" /></p>
              </div>
            </div>
            ...
            <div class="stat-card">
              ...
              <div class="stat-content">
                <h3>총 주문 수</h3>
                <p class="stat-number"><fmt:formatNumber value="${totalOrders}" pattern="#,###" /></p>
              </div>
            </div>
            ...
            <div class="stat-card">
              ...
              <div class="stat-content">
                <h3>오늘 주문 수</h3>
                <p class="stat-number"><fmt:formatNumber value="${todayOrders}" pattern="#,###" /></p>
              </div>
            </div>
          </div>
                      ...
                      <c:forEach var="product" items="${recentProducts}">
                        <li class="item">
                          <img 
                            ...
                            alt="${product.name}"
                            class="item-image"
                            onerror="this.src='https://picsum.photos/seed/product${product.productId}/50/50'"
                          />
                          <div class="item-info">
                            <h4>${product.name}</h4>
                            <p class="price"><fmt:formatNumber value="${product.price}" pattern="#,###" />원</p>
                            <c:if test="${product.createdAt != null}">
                              <p><fmt:formatDate value="${product.createdAt}" pattern="yyyy-MM-dd" /></p>
                            </c:if>
                          </div>
                        </li>
                      </c:forEach>
                      ...
                      <c:forEach var="farm" items="${recentFarms}">
                        <li class="item">
                          <img 
                            ...
                            alt="${farm.name}"
                            class="item-image"
                            onerror="this.src='https://picsum.photos/seed/farm${farm.farmId}/50/50'"
                          />
                          <div class="item-info">
                            <h4>${farm.name}</h4>
                    ...
                    <c:forEach var="category" items="${popularCategories}">
                      <div class="category-item">
                        ...
                        <div class="category-bar">
                          <div class="category-fill" style="width: ${(category.count / maxCount) * 100}%"></div>
                        </div>
