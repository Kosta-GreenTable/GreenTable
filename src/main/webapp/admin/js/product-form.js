...
function setupImagePreviews() {
  ...
  // 추가 이미지 미리보기 (update form)
  for (let i = 1; i <= 3; i++) {
    const additionalImageInput = document.getElementById("image" + i);
    const additionalImagePreview = document.getElementById(
      "additionalImagePreview" + i
    );
  ...
  // 대표 이미지 미리보기 (insert form - 기존 호환성)
  const mainImageInputOld = document.getElementById("productImage0");
  const mainImagePreviewOld = document.getElementById("mainImagePreview");

  if (mainImageInputOld && mainImagePreviewOld) {
    mainImageInputOld.addEventListener("change", function () {
      previewImage(this, mainImagePreviewOld.querySelector("img"));
    });
  }

  // 추가 이미지 미리보기 (insert form - 기존 호환성)
  for (let i = 1; i <= 3; i++) {
    const additionalImageInputOld = document.getElementById("productImage" + i);
    const additionalImagePreviewOld = document.getElementById(
      "additionalImagePreview" + i
    );

    if (additionalImageInputOld && additionalImagePreviewOld) {
      additionalImageInputOld.addEventListener("change", function () {
        previewImage(this, additionalImagePreviewOld.querySelector("img"));
      });
    }
  }
}
    ...
    cancelBtn.addEventListener("click", function () {
      ...
      if (confirm(confirmMessage)) {
        window.location.href =
          cancelBtn.getAttribute("data-return-url") ||
      ...
      for (const input of imageInputs) {
        if (input && input.files && input.files[0]) {
          ...
          if (!validTypes.includes(file.type)) {
            alert(
              "유효한 이미지 파일이 아닙니다. JPG, PNG, GIF 형식의 이미지를 선택해주세요."
            );
            input.focus();
            isValid = false;
            e.preventDefault();
