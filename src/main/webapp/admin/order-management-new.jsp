        ...
        <main class="admin-content">
            ...
            <div class="order-management">
                ...
                <div class="search-section">
                    <form class="search-form" action="${pageContext.request.contextPath}/front" method="get" id="searchForm">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <c:choose>
                                            ...
                                            <c:when test="${order.status == 'PROCESSING'}">
                                                <span class="status-badge processing">처리중</span>
                                            </c:when>
                                            <c:when test="${order.status == 'SHIPPED'}">
                                                <span class="status-badge shipped">배송중</span>
                                            </c:when>
                                            <c:when test="${order.status == 'DELIVERED'}">
                                                <span class="status-badge delivered">배송완료</span>
                                            </c:when>
                                    ...
                                    <td>
                                        <div class="action-btns">
                                            <button class="btn btn-primary" onclick="showOrderDetail('${order.orderNo}')">
                                                <i class="fas fa-eye"></i> 상세
                                            </button>
                                            <c:if test="${order.status != 'CANCELLED' && order.status != 'DELIVERED'}">
                                                <button class="btn btn-warning" onclick="showStatusModal('${order.orderNo}', '${order.status}')">
                        ...
                        <div class="pagination">
                            ...
                            <c:forEach begin="1" end="${totalPages}" var="i">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <a href="?key=admin&methodName=orderList&page=${i}&searchType=${searchType}&searchKeyword=${searchKeyword}&status=${status}&startDate=${startDate}&endDate=${endDate}">${i}</a>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
    ...
    <div id="statusModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title">주문 상태 변경</h3>
                <span class="close">&times;</span>
            </div>
            <form>
                <input type="hidden" id="orderNo" />
                <div class="form-group">
                    <label>주문번호: <span id="modalOrderNumber"></span></label>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="statusMemo">메모 (선택사항)</label>
                    <textarea id="statusMemo" placeholder="상태 변경 사유를 입력하세요"></textarea>
                </div>
                <div class="modal-actions">
                    <button type="button" class="btn btn-primary" onclick="updateOrderStatus()">변경</button>
                    <button type="button" class="btn btn-secondary" onclick="document.getElementById('statusModal').style.display='none'">취소</button>
                </div>
            </form>
        </div>
    </div>
