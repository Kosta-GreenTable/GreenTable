...
<html lang="ko">
  <head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/product-detail.css" />    <script>
  ...
  <body>
    ...
    <!-- 상품 이미지 및 정보 섹션 -->
    <section class="product-info-container">      <div class="product-image-container">
      ...
      <div class="product-info">
        <h1>${product.name}</h1>        <div class="price">
          <c:choose>
            <c:when test="${product.discountRate > 0}">              <div class="original-price-line">
        ...
        <div class="details">          <table>
          ...
          <div class="stock-alert">재고가 부족합니다.</div>
        </div>
        ...
        <div class="button-container">
          <button class="add-to-cart" data-product-id="${product.productId}">장바구니 담기</button>
          <button class="buy-now" data-product-id="${product.productId}">바로 구매하기</button>
        </div>
      </div>
    </section>    <!-- 리뷰 섹션 -->
    ...
    <section class="detail-section">
      ...
      </div>      <div class="tab-content">        <h3>${product.name}</h3>
        <p>${productDetail.description}</p>
        <div class="product-details-image">
          <c:if test="${not empty productImages && productImages.size() > 0}">
            <c:forEach var="image" items="${productImages}">
              <div class="detail-image">
                <img src="${pageContext.request.contextPath}/assets/images/products/${image.imageName}" alt="상품 상세 이미지" />
              </div>
            </c:forEach>
          </c:if>
              ...
              <div class="review-content">
                ...
                <c:if test="${not empty review.imageName}">
                  <div class="review-image">
                    <img src="${pageContext.request.contextPath}/assets/images/reviews/${review.imageName}" alt="리뷰 이미지" />
                  </div>
                </c:if>
              </div>
            </div>
      ...
      <div class="tab-content" style="display: none">
        <div class="delivery-info">
          ...
          <ul>
            ...
            <li>
              새벽 배송: 수도권 지역 새벽 배송 가능 (전날 오후 6시까지 주문 시)
            </li>
          </ul>
        </div>
        <div class="refund-info">
          ...
          <ul>
            ...
            <li>
              제품 하자, 오배송의 경우 수령 후 24시간 이내 고객센터로 연락
              주시면 교환/환불 가능합니다.
            </li>
            <li>
              식품 관련 문제 발생 시 사진과 함께 고객센터로 문의해 주세요.
            </li>
          </ul>
        </div>
      </div>
      ...
      <div class="product-list">
        <c:forEach var="recommended" items="${recommendedProducts}">
          <div class="product-card">
            <img src="${pageContext.request.contextPath}/assets/images/products/${recommended.mainImageName}" alt="${recommended.name}" />
